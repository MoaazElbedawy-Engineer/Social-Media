...
<div style={{position: "fixed", inset: 0, zIndex: 9999}} className="min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
    ...
    <div className="flex mt-4 gap-2">
        {
            bgColors.map((color)=>(
                ...
                onClick={() => setBackground(color)} style={{backgroundColor:color}} />
            ))
        }
    </div>

    <div className="flex gap-2 mt-4">
        <button onClick={()=>{setMode("text"); setMedia(null); setPreviewUrl(null)}} className={`p-2 rounded ${mode === "text" ? "bg-white text-black" : "bg-zinc-800"}`}>
            <TextIcon size={18} />
        </button>
